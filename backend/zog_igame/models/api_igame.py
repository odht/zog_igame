...
class Table(models.Model):
    ...
    '''
    @api.model
    def create_own_table(self,deal_qty):
        pid = self.env.user.partner_id
        self = self.sudo()
        d = self.env['og.deal']
        b = self.env['og.board']
        for i in range(1,deal_qty+1):
            deal_val = {'number':i}
            d_tmp= self.env['og.deal'].create(deal_val)
            d +=d_tmp
            board_val = {'deal_id':d_tmp.id}
            b += self.env['og.board'].create(board_val)
        vals={'partner_ids':pid,'name':pid.name + "'s table"}
        table = self.env['og.table'].create(vals)
        table.deal_ids = d
        table.board_ids = b
        name = 'channel of'+pid.name
        mail_channel = self.env['mail.channel'].create({'name':name})
        channel = self.env['og.channel'].create({'table_id':table.id,
                                             'mail_channel_id':mail_channel.id})
        return {'table_id':table.id,'channel_id':channel.id}
    
    @api.multi
    def table_org(self,mail_channel_id,pos1,pos2,pos3,pos4):
        self = self.sudo()
        s = [pos1,pos2,pos3,pos4]
        for item in s:
            vals ={'table_id':self.id,'partner_id':item['partner_id'],'position':item['position']}
            self.env['og.table.partner'].create(vals)
            channel_val = {'channel_id':mail_channel_id,'partner_id':item['partner_id']}
            self.env['mail.channel.partner'].create(channel_val)

        return True

    @api.model
    def get_indv_table(self):
        pid = self.env.user.partner_id
        self = self.sudo()
        tables = []
        table_partner = self.env['og.table.partner'].search([('partner_id','=',pid.id)])
        for rec in table_partner:
            table_info = {'name':rec.table_id.name,'id':rec.table_id.id,
                   'board':rec.table_id.board_ids.mapped('id')}
            tables.append(table_info)
        return tables
    

    @api.multi
    def next_sets_deal(self):
        self = self.sudo()
        table_id = self.id
        max_num = max(self.board_ids.mapped('number'))
        vals = {'number':max_num+1}
        deal = self.env['og.deal'].create(vals)
        board_vals = {'deal_id':deal.id,'table_id':table_id}
        board = self.env['og.board'].create(board_vals)
        self.deal_ids += deal
        self.board_ids += board
        return board.id
