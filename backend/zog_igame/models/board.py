...
class Board(models.Model):
    ...
    def _check_ready(self):
        for rec in self:
            if (rec.north_ready =='R' and rec.south_ready =='R'
    ...
    north_claimed = fields.Selection([('C','claimed'),('N','not claimed'),('O','once claimed')])
    south_claimed = fields.Selection([('C','claimed'),('N','not claimed'),('O','once claimed')])
    east_claimed = fields.Selection([('C','claimed'),('N','not claimed'),('O','once claimed')])
    west_claimed = fields.Selection([('C','claimed'),('N','not claimed'),('O','once claimed')])
    claim_agreed =fields.Selection([('Y','claim agreed'),('N','claim disagreed'),('O','once claimed')],compute = '_check_claim_agreed')
    ...
    #@api.onchange('north_claimed','south_claimed','east_claimed','west_claimed')
    @api.multi
    def _check_claim_agreed(self):
        ...
        channel = self.env['og.channel'].search([('table_id','=',table.id),('type','=','all')])
        sql = '''
             update og_board set claim_result = 0, west_claimed = 'O', east_claimed = 'O',claim_state = 'N' where id=%s
             '''
        sql1 = '''update og_board set claim_result = 0, north_claimed = 'O', south_claimed = 'O' ,claim_state = 'N' where id=%s'''
        if (self.north_claimed == 'O' and self.south_claimed == 'O') or (self.east_claimed == 'O' and self.west_claimed == 'O'):
            claim_agreed = 'O'  
        ...
        if self.declarer== 'N' or self.declarer == 'S':
            ...
            if self.east_claimed=='N' or self.west_claimed =='N':
                ...
                self.claim_result = 0
                idd= self.env['og.board'].browse(self.id)
