...
export default class HomeHome extends Component {
    ...
    render() {
        const data = [
            ...
            '正式域名备案中，请留存http://www.odooht.com;',

            '成绩查询网址：http://www.odhtqyy.com',
            '进主页后，在“活动”模块，点击“赛事查询入口”按钮,成绩实时直播;',
            ...
            '1、 比赛期间，各桌使用计分器录入成绩;',
            '2、 各桌左上角有本桌的登录账号和密码;',
            '3、 开室各桌登录账号为 100 + 桌号，如 101、102... , 闭室各桌登录账号为 200 + 桌号，如201、202、203...;',
            '4、 各桌密码为7位。账号 + 点 + 账号，如账号101的密码是101.101;',
            '5、 每一轮赛前，仔细核对自己所在牌桌，确定账号与桌号对应, 登录成功后，录分界面显示: 开闭室,桌号,赛段,轮次, 主队号,客队号;',
                '如：open, 1, A组循环赛.1,  2 vs 4。表示开室1号桌A组第1轮, 2号队 对 4号队;',
            '6、 录入过程中，有录入错误需要修改，召请裁判协助解决;',
            '7、 每轮成绩录入完成，双方队员核对无误后，点击“提交审核”按钮,之后无法修改;',
            '8、 需要同时记录三联记分表！赛后务必自行结分，并向裁判长交计分表;',
            '9、 成绩有误，请向裁判长汇报。有争议时，一切以三联计分表为准!',
