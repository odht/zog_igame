 ...
 */
  ...
  play = (item) => {
    ...
    // this.board.forEach((item1)=>{
    //   if(item1.card==item.card){
    //     flag=false;
    //   }
    // })
    // if (flag){
  ...
  */
  ...
  //确定玩家的位置
  confirmDirection(dir){
    switch(dir){
      case 'east':
        return {east:2,south:3,west:0,north:1}
      case 'south':
        return {east:1,south:2,west:3,north:0}
      case 'west':
        return {east:0,south:1,west:2,north:3}
      case 'north':
        return {east:3,south:0,west:1,north:2}
      case 'E':
        return {east:2,south:3,west:0,north:1}
      case 'S':
        return {east:1,south:2,west:3,north:0}
